...
type outpoint struct {
	txId   []byte //nolint:staticcheck // TODO: struct field txId should be txID
	index  int
	amount int64
}

func (o *outpoint) TxId() []byte { //nolint:staticcheck // TODO: method TxId should be TxID
	return o.txId
}
...
func createTestNonces(
	...
	for index := range inputList.inputs {
		...
		var sessionId [32]byte //nolint:staticcheck // TODO: var sessionId should be sessionID
...
func TestPartiallySignedTransaction_SignV1(t *testing.T) {
	const (
		hexTx    = "0100000001706bcabdcdcfd519bdb4534f8ace9f8a3cd614e7b00f074cce0a58913eadfffb0100000000ffffffff022cf46905000000001976a914072b22dfb34153d4e084dce8c6655430d37f12d088aca4de8b00000000001976a914fded0987447ef3273cde87bf8b65a11d1fd9caca88ac00000000" //nolint:lll
		...
		encodedUserKey = "tprv8eJiUjHpVRyTUM1p4XDRUdRZPJLfud22swAv48my1MxaCZztUNRrWxmN6ycdd9a2xfJwLchq5jW9m2jkNpwruijwvygCv41e6YrsqUvw7hQ" //nolint:lll
	)
...
func TestPartiallySignedTransaction_SignV3(t *testing.T) {
	const (
		hexTx = "01000000014a4ca718419999e9bfb675dc9f7deff6b65512c11469a23d169038267cd097040100000000ffffffff02916067590000000017a91437a2fceeb0c454b22b427c34eb565d8b1dc953ed8797c400000000000017a9142b0cabe5d058bc3c58f8a656dec2601d117262538700000000" //nolint:lll
...
func TestPartiallySignedTransaction_SignV5(t *testing.T) {
	...
	inputList := &InputList{inputs: []Input{
		&input{
			outpoint: outpoint{
				...
				amount: 100000,
				txId: hexToBytes(
					"45035ee77a25e5b1cb5b3a91dbe80d37db77cc6c14bbb054979812125dc69f23",
				),
			},
	...
	for index := range inputList.inputs {
		verifyInput(
			...
			signedTx,
			inputTxRaw[index],
			inputList.inputs[index].OutPoint().Index(),
			index,
		)
	}
}
	...
	inputList := &InputList{inputs: []Input{
		&input{
			outpoint: outpoint{
				...
				amount: 100000,
				txId: hexToBytes(
					"db784dbb4b1fadd2e5c7ecef6a00afb5dcb8dff7a3e649f1e510b704ab2e287e",
				),
			},
			address: addresses.New(
				6,
	...
	inputList := &InputList{inputs: []Input{
		&input{
			outpoint: outpoint{
				...
				amount: 100000,
				txId: hexToBytes(
					"c4b43c54fa33b70212c6e4ac6fa5a25a041bfe0aac1c5a793b317276617dcc76",
				),
			},
			address: addresses.New(
				3,
		...
		&input{
			outpoint: outpoint{
				...
				amount: 100000,
				txId: hexToBytes(
					"4b4f198a96c5fee378c18f20d34a962031bd9ea759e82be9aedbb0fc5c9ff4b3",
				),
			},
			address: addresses.New(
				2,
		...
		&input{
			outpoint: outpoint{
				...
				amount: 100000,
				txId: hexToBytes(
					"4221882416f18f7d534ea757ebb87be84624270b63f73d6552a087166a625468",
				),
			},
			address: addresses.New(
				4,
		...
		&input{
			outpoint: outpoint{
				...
				amount: 100000,
				txId: hexToBytes(
					"5f1ecaa9d05be209edfe804afdabde5dd60b2572d0a18d00dcec0b0f959b8451",
				),
			},
			address: addresses.New(
				1,
				"m/schema:1'/recovery:1'/external:1/0",
				"mk5bbq9qZ6Hh6u34wxvA4Hs6cEc4AF2HfL",
			),
		},
		&input{
			outpoint: outpoint{
				...
				amount: 100000,
				txId: hexToBytes(
					"28f06b18512a7796040fe4b34d0c56966ae8325a8c4f6ad9b51319e99ecb697d",
				),
			},
			address: addresses.New(
				6,
		...
		&input{
			outpoint: outpoint{
				...
				amount: 100000,
				txId: hexToBytes(
					"1473956088fc55c3388aec1cbd4b49edc8d577640303b0567df0b76d03cf94e8",
				),
			},
			address: addresses.New(
				5,
...
func TestPartiallySignedTransaction_SignSubmarineSwapV1(t *testing.T) {
	const (
		hexTx = "01000000021a608c7d6e40586806c33b3b1036fbd305c37e9d38990d912cc02de7e7cec05e0000000000fffffffff18bce10875329410641316bf7c4d984e00780174b6983080e9225dc26e5bd8c0100000000feffffff01705bc0230000000017a91470fcbc29723c85fdbf9fb5189220f279e9be4508878f030000" //nolint:lll
...
func verifyInput(t *testing.T, signedTx *wire.MsgTx, hexPrevTx string, prevIndex, index int) {
	...
	txscript.DisableLog()
	// logger := btclog.NewBackend(os.Stderr).Logger("test") logger.SetLevel(btclog.LevelTrace)
	...
	prevOutFetcher := txscript.NewCannedPrevOutputFetcher(
		prevTx.TxOut[prevIndex].PkScript,
		prevTx.TxOut[prevIndex].Value,
	)
...
func TestPartiallySignedTransaction_SignIncomingSwap(t *testing.T) {
	const (
		hexTx = "" //nolint:lll
		...
		shortChanId      = uint64( //nolint:staticcheck // TODO: const shortChanId should be shortChanID
	...
	inputs := []Input{
		&input{
			...
			incomingSwap: inputIncomingSwap{
				...
				swapServerPublicKey: "03912b4cfbd725133cbc319b444c1dad96a8bb0fcf840adc28c8e05e84ecbaa89b", //nolint:lll
	...
	inputs := []Input{
		&input{
			...
			incomingSwap: inputIncomingSwap{
				...
				// Instead of taking these last 2 values from the local/client's DB we take them
				// from Houston's data
			},
		},
	}
...
func TestPartiallySignedTransaction_Verify(t *testing.T) {
	...
	const (
		...
		txIndex1          = 1
		txAmount1         = 100000000
		txIdHex1          = "ae57aa2bc55298cc79459db058a91a45bc4a5cd57c9a988ce4de31b64ac01ca5" //nolint:staticcheck // TODO: const txIdHex1 should be txIDHex1
		...
		txIndex2          = 0
		txAmount2         = 100000000
		txIdHex2          = "568cf910024b5353c9b2d31dd8c338d368045ffde5d7a3876f8249a71f5939df" //nolint:staticcheck // TODO: const txIdHex2 should be txIDHex2
		...
		hexTx2 = "01000000010ead2fa0d6866d0414aba97fd8f1b242fdc3d4c8e7771e40969402319b6e876b0000000000ffffffff02922988040000000017a914d1ac5d61107d2bef187d1aef5cfd3536f4fd5dbe87d6b2050100000000220020bac6de765432ee16e10ce268341062f8f5a417b15a7f6ee8fe903e6d7470f0f700000000" //nolint:lll

		txIndex3          = 0
		txAmount3         = 93266680
		txIdHex3          = "6b876e9b31029496401e77e7c8d4c3fd42b2f1d87fa9ab14046d86d6a02fad0e" //nolint:staticcheck // TODO: const txIdHex3 should be txIDHex3
		...
		hexTx4 = "" //nolint:lll
		...
		hexTx5 = "" //nolint:lll

		txIndex5          = 1
		txAmount5         = 100000000
		txIdHex5          = "907e3c0c82b36b11b8543c9e058fe6e23d5ad35881f776e1ca9049e622f2cf80" //nolint:staticcheck // TODO: const txIdHex5 should be txIDHex5
		...
		hexTx6 = "" //nolint:lll

		txIndex6          = 1
		txAmount6         = 100000000
		txIdHex6          = "69391b987ec374f8e61a5fabb94899cd5efe802ee0f4d890bbbdbd18b05cac0f" //nolint:staticcheck // TODO: const txIdHex6 should be txIDHex6
		...
		hexTx7 = "0100000001a51cc04ab631dee48c989a7cd55c4abc451aa958b09d4579cc9852c52baa57ae0100000000ffffffff01a086010000000000220020452f4ae303ec79acd2bce8f7ddb6469f1060d9146003ea34887e5bbdf021c78700000000" //nolint:lll

		txIndex7          = 1
		txAmount7         = 100000000
		txIdHex7          = "383136ae84ddd35059a087fb56571f4809d97f09c04eac779ba31f6121818461" //nolint:staticcheck // TODO: const txIdHex7 should be txIDHex7
	...
	txId1, _ := hex.DecodeString(txIdHex1) //nolint:staticcheck // TODO: var txId1 should be txID1
	txId2, _ := hex.DecodeString(txIdHex2) //nolint:staticcheck // TODO: var txId2 should be txID2
	txId3, _ := hex.DecodeString(txIdHex3) //nolint:staticcheck // TODO: var txId3 should be txID3
	txId5, _ := hex.DecodeString(txIdHex5) //nolint:staticcheck // TODO: var txId5 should be txID5
	txId6, _ := hex.DecodeString(txIdHex6) //nolint:staticcheck // TODO: var txId6 should be txID6
	txId7, _ := hex.DecodeString(txIdHex7) //nolint:staticcheck // TODO: var txId7 should be txID7
	...
	// have 100000000 as input say 100_000 as change destination
	...
	secondInputGeneratingDust := input{
		outpoint: outpoint{
			index:  txIndex2,
			amount: 120000000 - txAmount1 + 122200 + 100, /* dust */
			txId:   txId2,
		},
		address: addresses.New(txAddressVersion2, txAddressPath2, txAddress2),
	}
