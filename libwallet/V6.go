...
// CreateAddressV6 returns a P2TR MuunAddress using Musig with the signing and cosigning keys.
func CreateAddressV6(userKey, muunKey *HDPublicKey) (MuunAddress, error) {
	return addresses.CreateAddressV6(
		&userKey.key,
		&muunKey.key,
		userKey.Path,
...
func (c *coinV6) FullySignInput(index int, tx *wire.MsgTx, userKey, muunKey *HDPrivateKey) error {
	derivedUserKey, err := userKey.DeriveTo(c.KeyPath)
	if err != nil {
		return errors.Errorf("failed to derive user private key: %w", err)
	}
	...
	userPubNonce, err := musig.MuSig2GenerateNonce(
		musig.Musig2v100,
		c.UserSessionId[:],
		userEcPriv.PubKey().SerializeCompressed(),
	)
	if err != nil {
		return err
	}
