...
enum NotificationProcessingState {
    case started
    case completed
}
...
public class NotificationProcessor {
    ...
    // Stream to track when the processing of a notification batch STARTS and COMPLETES
    ...
    public func poll() -> Completable {
        ...
        queue.async {
            do {
                self.processingSubject.onNext(.started)
            ...
            self.processingSubject.onNext(.completed)
            subject.onCompleted()
        }
