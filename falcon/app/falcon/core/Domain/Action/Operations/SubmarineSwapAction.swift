...
public class SubmarineSwapAction: AsyncAction<SubmarineSwapCreated> {
    ...
    public func run(invoice: String, origin: String) {
        ...
        let submarineSwapRequest = SubmarineSwapRequest(
            invoice: invoice,
        ...
        runSingle(
            houstonService.createSubmarineSwap(submarineSwapRequest: submarineSwapRequest)
                .map({ swapCreated in
                    ...
                    self.verifyLockTime(
                        swapCreated.swap,
                        expirationInBlocks: swapExpirationInBlocks
                    )
   ...
   func verifyLockTime(_ submarineSwap: SubmarineSwap, expirationInBlocks: Int) {
        /*
         ...
         Clients should check that the locktime in the funding output script for
         the swap is less than or equal to current_blockchain_height +
         previously_chosen_blocks_until_expiration + 2. The + 2 is to prevent a race
         condition if the server finds out about new blocks before the client.
         */
        ...
        if let userLockTime = submarineSwap._fundingOutput._userLockTime {
            if currentBlockchainHeight + expirationInBlocks + 2 < userLockTime {
                Logger.fatal(
                """
                This means we have a mismatch between the client information \
                and the server about the blockchain tip.
