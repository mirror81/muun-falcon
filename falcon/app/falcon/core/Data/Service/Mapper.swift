...
extension Client: APIConvertible {
    func toJson() -> ClientJson {
        ClientJson(
            ...
            iosAppId: appId,
            iosAppName: appName,
            iosAppPrimaryIconHash: appPrimaryIconHash,
            iosIsSoftDevice: isSoftDevice,
            iosSoftDeviceName: softDeviceName,
            iosHasGyro: hasGyro,
            iosInstallSource: installSource
        )
    }
}
...
extension RawTransactionResponseJson: ModelOperationConvertible {
    ...
    func toModel(decrypter: OperationMetadataDecrypter) -> RawTransactionResponse {
        return RawTransactionResponse(
            ...
            feeBumpFunctions: self.feeBumpFunctions.toModel()
        )
    }
...
extension OperationJson: ModelOperationConvertible {
    ...
    func toModel(decrypter: OperationMetadataDecrypter) -> Operation {
        ...
        var metadata: OperationMetadataJson? = nil
        do {
            metadata = try decrypter.decrypt(operation: self)
        } catch {
...
extension FeeBumpFunctionsJson: ModelConvertible {

    public func toModel() -> FeeBumpFunctions {
        return FeeBumpFunctions(uuid: uuid, functions: functions)
    }
}
...
extension RealTimeFeesJson: ModelConvertible {

    public func toModel() -> RealTimeFees {

        let targetedFees = targetFeeRates.confTargetToTargetFeeRateInSatPerVbyte
            .mapValues { FeeRate(satsPerVByte: Decimal($0)) }

        // Map from [Int: FeeRate] to [UInt: FeeRate] to meet FeeWindow
        var convertedTargetedFees = [UInt: FeeRate]()
...
extension FulfillmentPushedJson: ModelConvertible {

    func toModel() -> FulfillmentPushed {
        return FulfillmentPushed(
            nextTransactionSize: nextTransactionSize.toModel(),
            feeBumpFunctions: feeBumpFunctions.toModel()
        )
    }
}
