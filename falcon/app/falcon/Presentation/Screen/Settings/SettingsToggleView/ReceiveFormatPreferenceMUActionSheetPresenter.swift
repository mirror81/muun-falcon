...
class ReceiveFormatOptionsRetriever {
    static func run(selectedOption: any MUActionSheetOption) -> [MUActionSheetOptionViewModel] {
        let types = [
            MUActionSheetOptionViewModel(
                type: ReceiveFormatPreferenceViewModel.ONCHAIN,
                status: getStatus(
            ...
            MUActionSheetOptionViewModel(
                type: ReceiveFormatPreferenceViewModel.LIGHTNING,
                status: getStatus(
                    selectedOption: selectedOption,
                    enabled: true,
                    type: .LIGHTNING
            ...
            MUActionSheetOptionViewModel(
                type: ReceiveFormatPreferenceViewModel.UNIFIED,
                status: getStatus(
                    selectedOption: selectedOption,
                    enabled: true,
                    type: .UNIFIED
    ...
    private static func getStatus(
        ...
        enabled: Bool,
        type: ReceiveFormatPreferenceViewModel
    ) -> MUActionSheetCard
                                  .Status {
        guard let receiveFormatVM = selectedOption as? ReceiveFormatPreferenceViewModel else {
            let optionName = selectedOption.name
            Logger.fatal(
                "receive format is working with something that is not "
                    + "an ReceiveFormatVM \(optionName)"
            )
        }
