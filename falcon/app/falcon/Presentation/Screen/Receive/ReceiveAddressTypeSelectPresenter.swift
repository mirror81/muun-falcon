...
class AddressTypeOptionsRetriever {
    ...
    private func getStatus(
        ...
        enabled: Bool,
        type: AddressTypeViewModel
    ) -> MUActionSheetCard.Status {
...
extension AddressTypeOptionsRetriever {
    func run(selectedOption: any MUActionSheetOption) -> [MUActionSheetOptionViewModel] {
        var types = [
            MUActionSheetOptionViewModel(
                type: AddressTypeViewModel.legacy,
                status: getStatus(
            ...
            MUActionSheetOptionViewModel(
                type: AddressTypeViewModel.segwit,
                status: getStatus(
                    selectedOption: selectedOption,
                    enabled: true,
                    type: .segwit
        ...
        let taprootStatus = userActivatedFeatureSelector
            .get(for: Libwallet.userActivatedFeatureTaproot()!)
        switch taprootStatus {
        case .active:
            types.append(MUActionSheetOptionViewModel(
                type: AddressTypeViewModel.taproot,
                status: getStatus(
                    selectedOption: selectedOption,
                    enabled: true,
                    type: .taproot
